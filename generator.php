<?php
	...
	if(!isset($cmd))
		...
		switch ($argv[3]) {
			case 'Me' :
				$Me_generator = new Model_entity_generator($argv[2]);
				$Me_generator->generate();
				break;
			case 'ME' :
				$M_generator = new Model_generator($argv[2]);
				$E_generator = new Entity_generator($argv[2]);
				$M_generator->generate();
				$E_generator->generate();
				break;
			case 'CMe' :
				$C_generator = new Controller_generator($argv[2]);
				$Me_generator = new Model_entity_generator($argv[2]);
